...
export interface Roof extends MaterialType {}

export const createGetRoofs: (db: DB) => { query: () => Promise<Roof[]> } = (
  db: DB
) => {
  return {
    query: async (): Promise<Roof[]> => {
      ...
      const roofs = res.rows.map(roof => {
        const roofType = roofChoices.find(Roofp => {
          return Roofp.type === roof.TypeName;
        });
        assert(roofType, "no price found for " + roof.TypeName);
        const calc =
          roofType &&
          roofType.materials.reduce((prev, curr) => {
            const materialType = materials.find(
              materialp => materialp.type === curr.type
            );
