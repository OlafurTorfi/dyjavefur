...
export interface Wall extends MaterialType {
  purpose: string;
  comments: string;
}
...
export const createGetWalls: (db: DB) => { query: () => Promise<Wall[]> } = (
  db: DB
) => {
  return {
    query: async (): Promise<Wall[]> => {
      ...
      const walls = res.rows.map(wall => {
        const wallType = wallChoices.find(wallp => {
        ...
        const calc = wallType.materials.reduce(
          (prev, curr) => {
